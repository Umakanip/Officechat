...
import { AppBar, Toolbar, IconButton, Typography, Box, Container, Paper, List, ListItem, ListItemText } from '@mui/material';
import CallIcon from '@mui/icons-material/Call';
import MicIcon from '@mui/icons-material/Mic';
import MicOffIcon from '@mui/icons-material/MicOff';
import LeaveIcon from '@mui/icons-material/ExitToApp';
import AgoraRTC, { ILocalAudioTrack, IAgoraRTCClient, UID, IAgoraRTCRemoteUser, IRemoteAudioTrack } from 'agora-rtc-sdk-ng';
...
const AgoraVoiceChat: React.FC = () => {
  ...
  const initRtc = useCallback(async (roomId: string) => {
    try {
      ...
      const response = await axios.post('http://localhost:3000/api/calls', {
        CallerID: rtcUid,
        ReceiverID: null,
        GroupID: null,
        StartTime: startTime,
  ...
  const handleUserPublished = useCallback(async (remoteUser: IAgoraRTCRemoteUser, mediaType: 'audio' | 'video') => {
    if (rtcClient && mediaType === 'audio') {
      try {
        ...
        if (remoteAudioTrack) {
          ...
          console.error('Failed to get remote audio track for user:', remoteUser.uid);
        }
      } catch (error) {
  ...
  const handleUserLeft = useCallback((user: { uid: UID }) => {
    ...
    setMembers((prevMembers) => prevMembers.filter((member) => member.uid !== user.uid.toString()));
  ...
  const leaveRoom = useCallback(async () => {
    ...
    if (callID) {
      ...
      await axios.put(`http://localhost:3000/api/calls/${callID}`, { EndTime: endTime });
  ...
  useEffect(() => {
    const checkAudioTrack = async () => {
      try {
        if (audioTracks.localAudioTrack) {
          const volumeLevel = await audioTracks.localAudioTrack.getVolumeLevel();
          setIsMuted(volumeLevel === 0);
          console.log(`Local audio track is ${volumeLevel === 0 ? 'muted' : 'unmuted'}`);
        }
      } catch (error) {
  ...
  return (
    <Container maxWidth="sm">
      <AppBar position="static" sx={{ boxShadow: '0px 2px 4px rgba(0, 0, 0, 0.1)', backgroundColor: '#ffffff' }}>
        <Toolbar sx={{ display: 'flex', justifyContent: 'space-between' }}>
          <Typography variant="h6" component="div">
            Office Chat
          </Typography>
          <Box>
            <IconButton aria-label="audio call" onClick={enterRoom} disabled={roomJoined}>
              <CallIcon />
            </IconButton>
          </Box>
        </Toolbar>
      ...
      <Paper elevation={3} sx={{ marginTop: 2, padding: 2 }}>
        <Box sx={{ display: roomJoined ? 'flex' : 'none', flexDirection: 'column', alignItems: 'center', marginBottom: 2 }}>
...
// import MicOffIcon from '@mui/icons-material/MicOff';
// import LeaveIcon from '@mui/icons-material/ExitToApp';
// import AgoraRTC, { ILocalAudioTrack, IRemoteAudioTrack, IAgoraRTCClient, UID, IAgoraRTCRemoteUser } from 'agora-rtc-sdk-ng';
// import appid from '../../config';
...
//     if (rtcClient && mediaType === 'audio') {
//       try {
//         // Subscribe to the user's audio track using the IAgoraRTCRemoteUser object
//         await rtcClient.subscribe(remoteUser, mediaType);
  
//         // Extract and set the remote audio track
//         const remoteAudioTrack = remoteUser.audioTrack;
//         if (remoteAudioTrack) {
//           remoteAudioTrack.play();
//           setAudioTracks((prev) => ({
...
//   // const handleUserPublished = useCallback(async (user: { uid: UID, audioTrack: IRemoteAudioTrack }, mediaType: 'audio' | 'video') => {
//   //   if (rtcClient && mediaType === 'audio') {
//   //     try {
